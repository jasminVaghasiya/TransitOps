...
export default class TripPolicy extends BasePolicy {
  ...
  async canCreate(actor, target, req) {
    ...
    if (driver.status !== 'Available') {
      return this.deny(`Cannot assign a driver in '${driver.status}' status to a new trip`, 'DRIVER_UNAVAILABLE');
    }
  ...
  async canUpdate(actor, trip, req) {
    ...
    if (trip.status === 'Draft') {
      ...
      // If changing to a new driver, ensure they are Available
      if (newDriverId && String(newDriverId) !== String(trip.driver)) {
        if (driver.status !== 'Available') {
          return this.deny(`Cannot assign a driver in '${driver.status}' status`, 'DRIVER_UNAVAILABLE');
        }
      } else {
